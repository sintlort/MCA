...
class alpha_vantage extends Controller
{
    ...
    {
        $data = t_data_alpha::where($request->category, $request->type, $request->value)->with('relation_symbol')->get()->sortBy('date');
        return response()->json(['code'=>200, 'message'=>'alphavantage data finance success','data'=>$data],200);
    }

    public function apiStaticSortwithDate(Request $request)
    {
        $data = t_data_alpha::where($request->category, $request->type, $request->value)->with('relation_symbol')->whereBetween('date',[$request->date1, $request->date2])->get()->sortBy('date');
        return response()->json(['code'=>200, 'message'=>'alphavantage data finance success','data'=>$data],200);
    }
}
